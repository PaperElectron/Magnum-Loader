...
function PluginBase(loaded, pluginOptions, Shared) {
  ...
  try {
    var humanName = validHumanName(loaded.moduleName, Shared.FrameworkOptions.prefix);
  ...
  this.computedOptions = buildConfig(this.declaredName, Shared.FrameworkOptions.parentDirectory, this.defaultOptions, pluginOptions[this.humanName])
  this.hooks = hooks;
  this.errors = validErrors(loaded.loaded.errors)
}

exports = module.exports = PluginBase;
exports.validConfigName = validHumanName;
...
function validHumanName(moduleName, prefix) {
  var prefix = prefix + '-'
  if(!moduleName) {
