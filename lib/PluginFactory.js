...
module.exports = function(Dependencies, pluginOptions, Shared){
  var instanceObjects = {
    ...
    GlobalOptions: Shared.GlobalOptions
  }
  ...
  var mergedPlugins = (function(){
    ...
    var externalPlugins = _.chain(Dependencies)
      .filter(function(dep) {
        if(dep.indexOf(Shared.loaderPrefix + '-') === 0) return dep
      })
      .map(function(dep){
        return {
          require: dep,
    ...
    if(Shared.additionalPluginDirectory){
      internalPlugins = _.map(fs.readdirSync(Shared.additionalPluginDirectory), function(file){
        return {
          require: path.join(Shared.additionalPluginDirectory, file),
          external: false,
