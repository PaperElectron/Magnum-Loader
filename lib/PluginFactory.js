...
var RawPlugin = require('./RawPlugin');
var fs = require('fs');
...
module.exports = function(Dependencies, Shared){
  ...
  //Loads plugins defined by the system.
  ...
  var mergedPlugins = (function(){
    ...
    return FrameworkPlugins.concat(externalPlugins.concat(internalPlugins))
  })();
  ...
  var RawPlugins = _.chain(mergedPlugins)
    .map(function(plugin){
      ...
      return unrollPlugin(plugin, Shared.FrameworkOptions.layers)
    })
    .flattenDeep()
    .filter(Boolean)
    .value()
