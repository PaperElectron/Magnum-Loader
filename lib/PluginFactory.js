...
var InstallerPlugin = require('./Plugin/PluginTypes/PluginInstaller');
...
var InjectorPlugin = require('./PluginState/PluginStates/injectorPlugin');
var NonePlugin = require('./PluginState/PluginStates/NonePlugin');
/**
      ...
      (function() {
        ...
        if(ovrPlugin){
          ...
          var message = plugin.moduleName + '#' +
            plugin.metadata.name + ' hooks being overridden by plugin ' +
            ovrPlugin.moduleName

          Shared.Loggers.FrameworkLogger.warn(message);
        }
      })();

      if(plugin.getType() === 'installer'){
        return new InstallerPlugin(plugin, instanceObjects);
      }
      // return new InjectablePlugin(plugin, instanceObjects);
      return new pickPluginType(plugin.getType(),plugin, instanceObjects)
...
function pickPluginType(type, plugin, shared){
  switch (type) {
    case 'none':
      return new NonePlugin(plugin, shared)
      break;

    default:
      return new InjectorPlugin(plugin, shared)
  }
}