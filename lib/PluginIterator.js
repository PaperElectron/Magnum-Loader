 ...
 *  Report
 */
PluginIterator.prototype.generateReport = function() {
  var self = this;
  if(!this.allValid){
    return new Promise(function(resolve, reject) {
      reject(new self.FrameworkErrors.InvalidPluginsError('Invalid plugins present'));
  ...
  // return this._processLayers(reverselayers, 'stopPlugin', null, stopFinished)
  self.SystemLogger.log(self.Output.titleAnnounce('Stopping Runtime plugins.'));
  return self.Sorted.map((plugin) => {
    // console.log(plugin);
    return {
      type: plugin.type,
      injectableParam: plugin.paramName,
      prefix: plugin.prefix,
      moduleName: plugin.moduleName,
      configName: plugin.configName,
      parentModule: plugin.parentModule,
      options: plugin.options,
      computedOptions: plugin.computedOptions,
