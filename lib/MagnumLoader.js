...
MagnumLoader.prototype.load = function() {
  ...
  return this.iterator.load()
    ...
    .catch(this.FrameworkErrors.PluginDependencyError, function(err){
      instance.Logger.error(err.plugin.humanName + ' Encountered error while creating dependencies. ** ' + err.message);
...
MagnumLoader.prototype.start = function() {
  ...
  return this.iterator.start()
    ...
    .catch(function(err) {
      instance.Logger.error(err.plugin.humanName + ' Encountered error while starting. ** ' + err.message);
      instance.emit('error', err)
    })
};
...
MagnumLoader.prototype.stop = function() {
  ...
  return this.iterator.stop()
    ...
    .catch(function(err) {
      instance.Logger.error(err.plugin.humanName + ' Encountered error while stopping. ** ' + err.message);
      instance.emit('error', err)
    })
    .finally(function(){
      instance.emit('stop')
