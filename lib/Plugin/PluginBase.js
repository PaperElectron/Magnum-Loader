...
PluginBase.prototype._getExternalOptions = function(pluginOptions){
  if(this.multiple) {
    if(_.isObject(pluginOptions[this.parentModule])) {
      return pluginOptions[this.parentModule][this.configName] || false
    }
    return false
  }

  return pluginOptions[this.configName] || false;

};

PluginBase.prototype._computeConfig = function(pluginOptions){

  var external = this._getExternalOptions(pluginOptions);
  if(this.options) {
    var config;
