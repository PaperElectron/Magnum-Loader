...
module.exports = function(Dependencies, Shared){
    ...
    .map(function(plugin){
      ...
      if(_.isArray(plugin.loaded)){
        return _.map(plugin.loaded, function(mPlugin){
          ...
          return new RawPlugin(multiplePlugin, Shared.FrameworkOptions.layers);
        })
      }
      return new RawPlugin(plugin, Shared.FrameworkOptions.layers);
    })
    .flatten()
    .filter(Boolean)
    .value()
  ...
  var plugins = _.chain(rawplugins)
    .map(function(plugin) {
      ...
      return new Plugin(plugin, instanceObjects);
