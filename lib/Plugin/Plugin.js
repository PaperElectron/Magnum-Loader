...
Plugin.prototype.setDependencies = function(Dependencies) {
  ...
  if(_.isArray(Dependencies)) {
    this.dependencies = _.map(Dependencies, function(dep) {
      return new Dependency(self.configName, dep.name, dep.load, dep.type, self.FrameworkErrors)
    });
    this.Logger.log('Added dependency ' + this.getDepNames() + '.');
    return
  }

  this.dependencies = new Dependency(this.configName, this.injectName, Dependencies, this.type, this.FrameworkErrors);
